...
def compress_map(game_map):
    ...
    while parsed_size < map_size:
        ...
        while parsed_level_size < segment_size * 3:
            # print("\tsegment {}".format(segment_idx))
    ...
    # print()
    # print(campaign_normalized)
    print(len(campaign_normalized))

    if len(campaign_normalized) > MAX_CAMPAIGN_FILE_SIZE:
        raise CampaignTooLargeError(len(campaign_normalized))
    else:
        return campaign_normalized

    # campaign_file = open(path, "wb")
    # campaign_file.write(bytearray(campaign))
